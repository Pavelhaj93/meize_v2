...
function getLocale(request: Request) {
  ...
  let languages = new Negotiator({ headers: negotiationHeaders }).languages();
...
export async function middleware(request: NextRequest) {
  ...
  let locale = getLocale(request) || routing.defaultLocale;
...
export const config = {
  matcher: [
    ...
    "/((?!api|_next|icons|assets|images|favicon.ico|robots.txt|sitemap.xml|public).*)",
